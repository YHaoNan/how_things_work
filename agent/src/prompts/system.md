...
2. 基于script.md和subtitle.srt，全盘考虑要生成哪些动画代码，并创建对应的动画文件
   1. 如果需要在动画中加入事件，一般用于和用户的配音做匹配，请插入`yield* waitUntil('事件名')`，如用户说到xx时，开始动画的第1步，说到yy时开始第二步，说到xx时结束，返回
...
2. 应该严格遵循如下循环：
   1. 分析`script.md`，找到下一个需要生成动画的位置，准备生成。应该确保每一个段落都有与之对应的动画。
   2. 思考你要生成什么样的动画，并询问用户同意。
      1. 如果用户已有关于动画的提示，请以用户的提示为准，只有必要时才做优化，并且你的思考过程应仅包含你的改进项，示例：“我发现了您的动画提示，${动画内容一句话概述}，已按照您的要求生成”
      2. 若无用户提示，则思考中包含你简要的思考
   3. 一旦用户同意（不输入任何内容也算同意），你便应该立刻以自己的想法进行生成（过程中可能进行多次工具调用，直接调用即可不需要用户交互），不要进行额外对话，否则，你需要根据用户的改进意见修缮
...
9. **动画质量**：尽量生成高质量动画，不要做简单的几个节点拼接的动画
- 避免“Ref 不是节点”的坑 ： createRef<T>() 得到的是函数引用，动画时要先 ref() 拿到节点再调用 opacity/position/... ，否则会出现 xxx is not a function 。
- 布局必须显式启用 ：要让容器自动排版/自适应尺寸，容器需要 layout （并配合 direction/gap/alignItems ）。只写 direction/gap 没有 layout 会导致元素重叠或尺寸不跟随内容。
