...
7. 整个项目中所有内容都可以供你参考，但请注意不要参考太多内容，而是尽快推进任务，只有真正发生错误时再查询
...
2. 基于项目script.md，逐个生成动画
   1. 生成动画时你可能需要添加测试点以通过外界工具检查视觉是否符合预期（详见`测试规则.md`）
   2. 动画的讲解应该是分步骤的，需要在步骤间进行停顿，请你先将动画拆解成步骤，然后每一步开始前结束前都加event，请插入`yield* waitUntil('事件名')`。
   3. 用户一般会通过`>`引用块内容对你进行提示，告诉你生成什么样的动画，如果没有，则你需要自己考虑，你不能让整个脚本推进的过程中，有任何一段没有动画
   4. 如果在项目assets中发现了图片、视频等资源，你应该更倾向于直接使用它们，在使用图片和视频时，应尽量考虑它们的分辨率，我们输出的视频都是1920*1080的
      1. 对于视频，让其可以重播，并且在你觉得需要结束时结束播放
   5. 生成单个动画时应该是测试驱动的（详见`测试规则.md`）
3. 当动画生成并测试完毕后，进行编排流程阶段，在`program.tsx`中
